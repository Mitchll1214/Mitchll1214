// /api/realm?user=xxx — 按注册时长自动定境的修仙档案卡（Vercel serverless 入口）
...
export async function renderRealm(username) {
  ...
  ${text({ x: 80, y: 101, s: `@${username} · 注册于 ${created.toISOString().slice(0, 10)}`, size: 11, fill: '#7A7393' })}
  ${linggen}
...
export default async function handler(req, res) {
  try {
    ...
    res.end(`realm 渲染失败: ${err.message}`);
  }
}
